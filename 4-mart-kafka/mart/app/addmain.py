...
async def kafka_consumer(topic:str, bootstrap_server:str):
    ...
    try:
        async for message in consumer:
            print(f"Recive message {message.value} on topic {message.topic}")
            # print(f"Recive message {message.value.decode()} on topic {message.topic}, decode func")
    finally:
        await consumer.stop()
...
async def lifespan(app:FastAPI):
    ...
    asyncio.create_task(kafka_consumer("mart-topic", "broker:19092"))
    print("table created")
    yield
...
async def add_product(product: ProductReq, session: Annotated[Session, Depends(get_session) ] ):
    ...
    try:
        await producer.send_and_wait("mart-topic", ready_product) 
    finally:
        await producer.stop()
...
async def order_place(order:Order, session: Annotated[Session, Depends(get_session)]):
    ...
    await producer.start()
    try:
        await producer.send_and_wait("mart-topic", new_order) 
    finally:
        await producer.stop()
