...
async def user_login(form_data, session: Session, producer: AIOKafkaProducer):
    ...
    user_token = UserTokenModel(
        created_at=datetime.now(timezone.utc),expired_at=datetime.now(timezone.utc) + timedelta(minutes=int(EXP_MIN)), token=access_token, user_id=user.id)
    session.add(user_token)
    session.commit()
    session.refresh(user_token)
    ready_token = SendUserToken(access_token=access_token, expire_in=str(EXP_MIN), token_type="Bearer")
    return ready_token