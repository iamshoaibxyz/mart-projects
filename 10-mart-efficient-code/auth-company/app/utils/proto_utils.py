...
#         tokens = [user_token_to_proto(token) for token in user.tokens]
#     if hasattr(user, 'orders'):
#         orders = [order_to_proto(order) for order in user.orders]
#     if hasattr(user, 'carts'):
#         carts = [cart_to_proto(cart) for cart in user.carts]
