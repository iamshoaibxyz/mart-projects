...
from app.schemas.product import ProductAddReq, ProductUpdateReq, ProductAddWithInventoryReq, ProductGetByIdReq, ProductsGetByCompanyIdReq, ProductModelSchema
...
async def add_product(product_data:ProductAddReq, token: Annotated[dict, Depends(oauth2_company_scheme)], session: Annotated[Session, Depends(get_session)]):
    ...
    product = ProductModel(name=product_data.name, category=product_data.category, description=product_data.description, price=product_data.price, company_id=company.id)
    
    proto_product = product_to_proto(product)    
    async with get_producer() as producer:
        await producer.send_and_wait("product-product-product-added", proto_product.SerializeToString())
    return { "company": company, "status": status.HTTP_201_CREATED, "message": "you have succcessfully added the product and we have send you an email for confirmation"}
