...
async def create_company(company: CompanyReq, session: Annotated[Session, Depends(get_session)]):
    ...
    context_str = str(new_company.get_context_str())
    hash_url = hashed_url(context_str)
    return {"hashed_url": hash_url, "status": status.HTTP_201_CREATED, "message": "you have succcessfully signed up the company and we have send you an email, please check and verify"}
